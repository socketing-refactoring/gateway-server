...
@Slf4j
@Component
public class JwtManagerAuthenticationFilter
        extends AbstractGatewayFilterFactory<JwtManagerAuthenticationFilter.Config> {

    private final PublicKey publicKey;

    public JwtManagerAuthenticationFilter(@Value("${token.jwt.public}") String publicKeyPem) {
        super(Config.class);
    ...
    public GatewayFilter apply(Config config) {
        ...
        return (exchange, chain) -> {
            ...
            try {
                ...
                log.info("JWT 검증 성공: managerId={}", managerId);

                // 헤더에 사용자 정보 추가
                ServerWebExchange modifiedExchange = setHeaders(exchange, managerId);
    ...
    private ServerWebExchange setHeaders(ServerWebExchange exchange, String managerId) {
        ServerHttpRequest modifiedRequest =
                exchange.getRequest().mutate().header("x-api-managerId", managerId).build();
