                ...
                .then(
                        Mono.fromRunnable(
                                () -> {
                                    ...
                                    headers.forEach(
                                            (key, value) -> log.debug("{}: {}", key, value));

                                    // 중복 제거 (Origin, Credentials)
                                    String allowHeaders =
                                            headers.getFirst("Access-Control-Allow-Headers");
                                    if (allowHeaders != null) {
                                        String cleanedAllowHeaders =
                                                allowHeaders.replaceAll("[\\[\\]]", "");
                                        String[] allowHeadersArray =
                                                cleanedAllowHeaders.split(",\\s*");

                                        headers.put(
                                                "Access-Control-Allow-Headers",
                                                List.of(allowHeadersArray[0]));
                                    }

                                    String allowCredentials =
                                            headers.getFirst("Access-Control-Allow-Credentials");
                                    if (allowCredentials != null) {
                                        String cleanedCredentialsHeaders =
                                                allowCredentials.replaceAll("[\\[\\]]", "");
                                        String[] allowCredentialsArray =
                                                cleanedCredentialsHeaders.split(",\\s*");

                                        headers.put(
                                                "Access-Control-Allow-Credentials",
                                                List.of(allowCredentialsArray[0]));
                                    }

                                    String allowOrigins =
                                            headers.getFirst("Access-Control-Allow-Origin");
                                    if (allowOrigins != null) {
                                        String cleanedAllowOrigins =
                                                allowOrigins.replaceAll("[\\[\\]]", "");
                                        String[] allowOriginsArray =
                                                cleanedAllowOrigins.split(",\\s*");

                                        headers.put(
                                                "Access-Control-Allow-Origin",
                                                List.of(allowOriginsArray[0]));
                                    }
                                    ...
                                    if (removed) {
                                        log.debug("Vary header was removed successfully.");
                                    } else {
                                        log.debug("No Vary header found.");
                                    }
